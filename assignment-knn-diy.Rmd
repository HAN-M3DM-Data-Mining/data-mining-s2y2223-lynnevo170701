...
The dimension of dataset is 768 x 9, which means that the data has 768 observations and 9 variables The data type of all variables are `num`. We can use `summary` function to see the summary information of each variable and we can check if each has Nas values, which we need to clean.
...
As shown in the graph, k = 11 will return the lowest error rate. 11 will be used as the amount of nearest neighbours `k` in `knn` function below.
